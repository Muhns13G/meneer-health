...
import { createMeasurementEventHttpHandler } from "@/server/measurement/measurement-http";
import {
...
export const Route = createFileRoute("/api/measurement/events")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          return createMeasurementEventHttpHandler(
