...
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/`                                     | Public responsive marketing page; transactional CTAs lead to gated flows and several retained claims still require domain evidence.                                                       | **Functional** public acquisition      | Retain public; redirect transactional CTAs only to the gated cohort entry; preserve established messaging and track substantiation under TD-006.                                                                                           |
...
| `/peptides`                             | First-rollout product with a gated transaction; preserved prototype contains local profile/password fields, placeholder acknowledgement, missing media, and a `.example.com` destination. | **Restricted** first-rollout pathway   | Keep the transaction gated with no data collection. Do not link to Precise Wellness until product-level, partner, questionnaire, transfer, escalation, and dispensing evidence is approved. Gating must not be presented as "coming soon." |
...
| Peptide navigation and treatment card                          | Navigate to `/peptides`.                                                                 | **Restricted** first-rollout pathway; retain established product messaging while exposing no profile, password, acknowledgement, or health-data capture. |
...
7. Complete the product-level Precise Wellness evidence before activating the gated peptide
   transaction; its owner-confirmed first-rollout position does not itself authorise activation.

## Implementation Order

1. Gate `/start` and replace all false transaction states with an explicit restricted-unavailable state.
2. Present `/peptides` as the first-rollout product with a gated, no-data transaction and disable its
   external hand-off while preserving the current implementation.
