...
signed webhook, and persists replay-safe audit and reconciliation evidence in local Supabase. No
customer route, hosted webhook, real Stripe account, production price, or live charge is enabled.
...
| Check                 | Result                                                                     |
| --------------------- | -------------------------------------------------------------------------- |
| Vitest                | 36 files / 215 tests passed                                                |
| pgTAP                 | 8 files / 257 tests passed; 46 payment-specific assertions                 |
| Signed integration    | Checkout, browser denial, paid event, replay, and tamper rejection passed  |
| Test charge scenarios | Consultation, medication/delivery, and three-line bundle snapshots passed  |
...
| Browser regression    | 52 Playwright desktop/mobile checks passed                                 |
| Recovery              | 82 of 82 encrypted backup and recovery checks passed in 9 seconds          |
| Build and deployment  | Production build, route-tree check, and Cloudflare dry run passed          |
...
TD-010 particulars are signed off. Record redacted event IDs, timestamps, alert outcomes, and
reconciliation results; never record keys, payment details, patient data, or raw webhook bodies.
